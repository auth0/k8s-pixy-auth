...
// base64URLEncode follows the nodejs implementation from https://auth0.com/docs/api-auth/tutorials/authorization-code-grant-pkce
// because they do the replacing which base64.URLEncoding.EncodeToString doesn't
func base64URLEncode(b []byte) string {
	encoded := base64.URLEncoding.EncodeToString(b)

	var pre = regexp.MustCompile(`\+`)
	encoded = pre.ReplaceAllString(encoded, "-")

	var sre = regexp.MustCompile(`\/`)
	encoded = sre.ReplaceAllString(encoded, "_")

	var ere = regexp.MustCompile(`=`)
	encoded = ere.ReplaceAllString(encoded, "")
...
// getAuthInfo
func getAuthInfo(domain, clientID, audience string) authorizationCodeResponse {
	...
	challenge := base64URLEncode(csum[:])
