...
5. Under the settings section of the newly created Application, add `http://127.0.0.1:8080/callback` to the `Allowed Callback URLs` section. This tool will be listening for a short period of time at that location when the user authenticates so it can handle receiving the needed information from Auth0.
